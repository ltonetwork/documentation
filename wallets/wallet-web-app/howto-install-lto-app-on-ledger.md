...
There are currently multiple ways to interact with the LTO Network blockchain using a Ledger hardware wallet: the [**Official Web Wallet**](./) and [**third-party wallets**](../third-party-wallets/) **** like stakely.io and an installable CLI Wallet**.**
...
The interface is exactly the same as described on the [Official Web UI](./) page. The only addition while using a Ledger device is that it requires to sign transactions by physically touching your Ledger device.
